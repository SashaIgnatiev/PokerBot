...
def main():
    ...
    print(get_opponent_chips(old_imgray, w, h))
    ...
    while (1):
        ...
        if turn == False:
            ...
            try:
                ...
                list_of_cards, state, update2 = update_list_of_cards(cards, list_of_cards, state)
                pos = get_dealer_pos(imcolor, w, h)
            except:
                pass

            #if the game state is being updated
            if update2 == True or old_dealer_pos != pos:

                #reset the bet_total, and poker bot fields
                bet_total = reset(pokerBot)

                #if the state is a preflop, add in the amount of chips that the opponent will put in to bet total
            ...
            print(f"da_chips: {pokerBot.chips}")
            pos = get_dealer_pos(imcolor, w, h)
            pot = grab_pot(imgray)

            print(f"bet before adjustment: {bet_total}")


            print(old_dealer_pos)
            print(pos)
            print(update)

            '''bet gets increased when the game state is not advancing, this occurs when:
                1: game state not being updated
                2: when the dealer remains the same
            '''

            #add on the oppoenent's bet to your bet, if there is an update, revert that change
            yes = get_opponent_bet(imgray, old_imgray, w, h)
            bet_total = bet_total + yes

            # if update == False or old_dealer_pos != pos:
            #     bet_total = reset(pokerBot)

            bet = bet_total - pokerBot.raise_amount


            print(f'BET: {bet_total}')
            print(f'POT: {pot}')
