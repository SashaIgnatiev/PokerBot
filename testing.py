...
def isolate_info(card):
    ...
    h = np.shape(card)[1]
    #try 22
    rank = get_box2(card, (24, 21), (0, 0), old_w = 49, old_h = 58, new_w = w, new_h = h)
    suite = get_box2(card, (26, 23), (22, 0), old_w = 49, old_h = 58, new_w = w, new_h = h)
...
def adjust_win(x, y, w, h):
    ...
    pyautogui.moveTo(corner_x, corner_y, 1)
    time.sleep(1)
    pyautogui.dragTo(x + 841, y + 608, 2, button='left')# move the mouse to 100, 200, then release the right button up.
...
def main():
    ...
    list_of_cards, state = update_list_of_cards(cards, list_of_cards)
    ...
    x = get_my_chips(imgray, w, h)
    print(x)

    #x = grab_pot(imgray)

    # rewrite = list_of_cards[0].suit_im
    # cv.imshow("yes", rewrite)
    # cv.waitKey(0)
    # cv.destroyAllWindows()
    #cv.imwrite("sample_images3/heart_first.png", rewrite)
