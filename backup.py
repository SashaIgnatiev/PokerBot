...
def process_table(img):
    ...
    output = get_box(img, (50, 312), (292, 317))
...
def isolate_info(card):
    rank = get_box(card, (24, 20), (0, 0))
    suite = get_box(card, (26, 24), (24, 0))
...
def main():
    name = 'Screen_Shot_2024-01-01_at_19.05.19.png'
    name = "PokerStars.net Morrison III - No Limit Hold'em 50/100 Play Money - 1 on 1 - Logged In as Destroyer1248"
    # loading sample images
    sample_ranks, sample_suits = load_samples()

    # grabbing a grayscale image of the poker window
    imgray = grab_poker_window(name)
    ...
    pass
    print("hello1")
    my_cards_color = cv.cvtColor(imgray, cv.COLOR_GRAY2RGB)

    # test1, test2 = process_my_cards(my_cards)
    # test_color = cv.cvtColor(test1, cv.COLOR_GRAY2RGB)

    rewrite = list_of_cards[0].rank_im
    # cv.imshow("yes", rewrite)
    # cv.imwrite("sample_images3/three.png", rewrite)
    # testing methods

    # cv.drawContours(my_cards_color, [approx_c2], -1, (0, 200, 0), 1)
    # cv.imshow("My Cards", my_cards_color)

    # cv.drawContours(my_cards_color, contour, -1, (0, 200, 0), 1)
    # cv.imshow("FirstRank", yes)
